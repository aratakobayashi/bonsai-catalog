...
-- Migration: 002_extend_gardens_table_fixed.sql
-- Fixed: Remove Japanese text search configuration
...
-- Create basic full-text search indexes (using default configuration)
CREATE INDEX idx_gardens_name_search ON gardens USING GIN(to_tsvector('simple', name));
CREATE INDEX idx_gardens_description_search ON gardens USING GIN(to_tsvector('simple', description));
CREATE INDEX idx_gardens_owner_message_search ON gardens USING GIN(to_tsvector('simple', owner_message));

...
import { ChevronLeft, ChevronRight, Clock, ShoppingBag, Calendar, ArrowRight } from 'lucide-react'
          ...
          {articles.map((article) => (
            <Card key={article.id} className="group overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
              ...
              <Link href={`/guides/${article.slug}`}>
                <div className="aspect-video relative overflow-hidden bg-gray-100 cursor-pointer">
              ...
              <CardContent className="p-6">
                ...
                {/* タイトル - クリック可能 */}
                <Link href={`/guides/${article.slug}`}>
                  <h2 className="font-bold text-lg mb-3 text-gray-900 hover:text-accent-600 transition-colors duration-200 line-clamp-2 leading-tight group-hover:text-accent-600 cursor-pointer">
                    {article.title}
                  </h2>
                </Link>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {/* 続きを読むボタン - クリック可能 */}
                  <Link href={`/guides/${article.slug}`}>
                    <Button variant="ghost" size="sm" className="text-xs hover:text-accent-600 transition-colors">
                      続きを読む
